...
$(document).ready(function() {
	...
	// printer button
	$('#printResultsButton').click(function() {
		window.print();
	});
...
function startRound() {
	...
	for ( var j = 0; j < numCards; j++) {
		$("#card" +j).flip({
					...
					onEnd : function() {
						$("#card" + curCard).hide();
						$("#space" + curCard).append(
								'<canvas id="distcanv' + curCard
										+ '"></canvas>');
						$("#distcanv" + curCard).css({
							"width" : "190",
							"background-color" : "white",
							"height" : "90"
						});
						cardList[curCard].setDist(graphTypes[curCard] + 1,
								curCard, masterParms);
						curCard++;
					}
				})
...
function swapNext() {
	if (swapsLeft > 0) {
		...
		do {
			c2 = Math.floor(Math.random() * numCards)
		} while (c2 == c1)
		...
		swapsLeft--;
		// swap cards
		swapCards("#space" + c1, "#space" + c2, (c1 * (190 + 40)),
				(c2 * (190 + 40)));
...
$("#facecard").flip({
								...
								color : "#008B8B",
								content : ("Select the "
										+ (cardList[correctCard].dist.getDist()) + " distribution")
							})
		} else {
$("#facecard").flip({
						direction : "tb",
						onEnd : function() {
							var histogram = new histMaker(
									graphTypes[correctCard] + 1, 500,
									correctCard, masterParms[correctCard])
							$("#flipsection").append(
									'<canvas id="hist' + correctCard
											+ '"></canvas>');
							histogram.initializeExperiment();
							$("#hist" + correctCard).css({
								"width" : "400",
								"background-color" : "white",
								"height" : "200"
							});
...
function endRound() {
	...
	if (answeredCard == correctCard) {
		...
		if (textMode)
			...
			$("#facecard").css({
				"background-color" : "rgb(50,200,60)"
			})
		...
		window
				.setTimeout(
						function() {
							...
							$("#stats").html(
									"<h4>Longest Streak:</h4><p>"
											+ longestStreak
											+ "</p><h4>Accuracy:</h4><p>"
											+ (p1score * 100 / numRounds)
													.toFixed(2) + "%</p>");
							if (p1score < numRounds / 2)
								...
								$("#finalmess")
										.html(
												"<b>Great Job! Try to get a perfect score next time!</b>");
							else
								$("#finalmess").html(
										"<b>Wow! Perfect Score!</b>");
							$('#results-modal').modal('show');

						}, 1000);
	}
	$("#scores").html(
			'<p>Score: ' + p1score + '</p><p>Current Round: '
					+ (numRounds - roundsLeft + 1) + '</p><p>Rounds Left: '
					+ (roundsLeft - 1) + '</p><p>Longest Streak: '
					+ longestStreak + '</p>')
...
function setBoard() {
	...
	$("#scores").html(
			'<p>Score: 0</p><p>Current Round: 1</p><p>Rounds Left: '
					+ numRounds + '</p><p>Longest Streak: 0</p>')
	$("#scores").css({
		"position" : "relative",
		"left" : (((textMode) ? 1000 : 400) + 15) + "px",
		"top" : "50px",
		"border-style" : "solid",
		"width" : "200px"
	})
	...
	$("#board").css({
		"position" : "relative",
		"top" : "110px",
		"width" : ((190 * (numCards) + 50 * (numCards - 1)) + "px"),
		"z-index" : "0",
		"height" : (110) + "px",
...
function animateAround(name, radius, startX, startY, dir, theta, speed,
		swapstring) {
	theta += .01;
	var nextX = (Math.cos(theta * Math.PI + Math.PI) + 1) * radius * dir
			+ startX;
	...
	if (theta < 1) {
		window.setTimeout(function() {
			animateAround(name, radius, startX, startY, dir, theta, speed,
					swapstring)
		}, 10 / speed)
	} else {
...
function swapCards(card1, card2, card1x, card2x) {
	animateAround(card1, (card2x - card1x) / 2, card1x, 90 * Number($(card1)
			.children().attr('id').substring(4,
					$(card1).children().attr('id').length)) - 30, 1, 0,
			gameSpeed, card2)
	animateAround(card2, (card2x - card1x) / 2, card2x, 90 * Number($(card2)
			.children().attr('id').substring(4,
					$(card2).children().attr('id').length)) - 30, -1, 0,
			gameSpeed, card1)
}