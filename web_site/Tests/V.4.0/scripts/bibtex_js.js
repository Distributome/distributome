...
function BibtexParser() {
  ...
  this.setRef = function(t) {
	  ...
	  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	  //
	  alert("BibtexParser.setRef::refName = " + this.refName);
	  ...
	  if(refName!=null){
	        ...
	        alert ("BibtexParser.setRef::tpl="+tpl);
...
function bibtex_js_draw(entryKeyList) {
	  $("#bibtex_display").hide();
	  ...
	  */
	 //alert("here entrykey::"+entryKey); 
	    var input = getBibtexHtml(bib_data, entryKeyList);
		//alert("input"+input);
	    (new BibtexDisplay()).displayBibtex(input, $("#bibtex_display"));
	    $("#bibtex_display").hide();
	...
	function getBibtexHtml(bibtexInput, entryKeyList){
		//alert("entrykeylist"+entryKeyList);
		if(entryKeyList == undefined) return bibtexInput;
		var list = entryKeyList.split(",");
		var html = '';
		for(var i=0; i<list.length; i++){
			html += getBibtexHtmlPerEK(bibtexInput, list[i]);
		}
		//alert(html);
		return html;
	}
	
	function getBibtexHtmlPerEK(bibtexInput, entryKey){
		var pos = bibtexInput.indexOf(entryKey);
		var book = bibtexInput.substring(pos-6, pos).trim();
		var article = bibtexInput.substring(pos-9, pos).trim();
		var misc = bibtexInput.substring(pos-6, pos).trim();
		var start, end;
		//alert("book::"+book+";article::"+article+";misc::"+misc);
		if(article == "@article{") start = pos-9;
		else start = pos-6;	
		
		var remainingString = bibtexInput.substring(pos);
		//alert(remainingString.length);
		//alert("remainingString"+remainingString);
		var end = getEndBibtex(remainingString);
		//alert("pos::"+pos+";start::"+start+";end::"+end);
		//alert(bibtexInput.substring(start,pos+end));
		return bibtexInput.substring(start,pos+end);
	}
	
	function getEndBibtex(input){
		var book = input.indexOf("@book{");
		var article = input.indexOf("@article{");
		var misc = input.indexOf("@misc{");
		return min(book,article,misc);
	}
	
	function min(book, article, misc){
		if(book==-1) book =9999;
		if(article ==-1) article =9999;
		if(misc ==-1) misc =9999;
		if(book<article){
			if(book<misc) return book;
			else return misc
		}else{
			if(article<misc) return article;
			else return misc;
		}
	}
	
	function getBibtexReferences(){
	}
