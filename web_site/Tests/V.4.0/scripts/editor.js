...

{
		...
		xmlhttp.open("GET","./data/Distributome.xsd",false);
		xmlhttp.send();
...
function editorSave() {
	...
	if (editor.data("type") == "distribution") {
		...
		//Updating the xmldoc 
		var distributionNode = createElement('distribution', '', null);
		var newNode = new Object();
		var index = distributome.nodes.length;
		connectivity = 	"top_hierarchy";		
		//newNode.selected = "top_hierarchy";
		newNode.selected = "red";
			...
			if (name != "id") {
				if (name == "name") {
					if (xId == "") {
						...
						//Update Arrays
						newNode.nodeName = camelize(trimDistribution(value));
						distributomeNodes[getDistributionName(value)] = index;
						distributomeNodes[index] = getDistributionName(value);
						
						
						/*distributome.nodes[index] = new Object();
						distributome.nodes[index].selected = "top_hierarchy";
						distributome.nodes[index].nodeName = camelize(trimDistribution(value));
						distributomeNodes[getDistributionName(value)] = index;
						distributomeNodes[index] = getDistributionName(value);
						distributome.nodes[index].group = 3;
						*/
						//updateXMLDoc();
						
						//alert(value+"   "+camelize(trimDistribution(value))+" dist name"+getDistributionName(value));
						//updateNodes();
					}
					...
					newNode.group = getGroup(value.toLowerCase());
				} else if (name == "cite"){					
					citeNodes[getDistributionName(value)] = index;
					if(!citeFalg){
						citeNodes[index] = trim(value);
						citeFlag = true;
					}else{
						citeNodes[index] = citeNodes[index]+ ","+trim(value);
					}
				}
				...
				//Add the child nodes
				var tagElement = createElement(name, '', distributionNode);
				var valueElement = createElement('text', value, tagElement);
			} else {
				xId = value;
			}
		});
		...
		restart();
		distributionNode.id = xId;
		updateXMLDoc("distribution", distributionNode);
		...
		//Updating the xmldoc 
		var relationNode = createElement('relation', '', null);
		var relations = new Array();
		var edgeIndex = distributome.edges.length;
		var fromCount = 0;
		var toCount = 0;
		var citeFalg = false;
		...
		editorContent.children().each(function(){
			var name = $(this).data("name").toLowerCase();
			if(name == "from") fromCount++;
			if(name == "to") toCount++;
		});
		var extraLinks = (fromCount > toCount ? fromCount : toCount);
		for(var i=0; i<extraLinks; i++){
		...
		editorContent.children().each(function(){
			...
			if (name != "id") {
				...
				if (name == "to") {
					to = value;
					value = value + " distribution";
					var toValue = getNodeIndex(distributomeNodes, getDistributionName(value));
					var currentIndex = toCount--;
					relations[currentIndex-1].target = distributome.nodes[toValue];
					for(var i=(fromCount-currentIndex); i>=0;i--){
						relations[i].target = distributome.nodes[toValue];
					}
					
				} else if (name == "from") {
					from = value;
					value = value + " distribution";
					var fromValue = getNodeIndex(distributomeNodes, getDistributionName(value));
					var currentIndex = fromCount--;
					relations[currentIndex-1].source = distributome.nodes[fromValue];
					for(var i=(toCount-currentIndex); i>=0;i--){
						relations[i].source = distributome.nodes[fromValue];
					}
				} else if (name == "statement") {
					statement = value;
				} else if (name == "type"){
					var type = getRelationStrength(value.toLowerCase());
					for(var extraNodes =0; extraNodes<extraLinks; extraNodes++){
						relations[extraNodes].value = type;
					}
					
				} else if (name == "cite"){
					if(!citeFalg){
						citeLinkNodes[edgeIndex] = trim(value);
						citeFlag = true;
					}else{
						citeLinkNodes[edgeIndex] = citeLinkNodes[edgeIndex]+ ","+trim(value);
					}
				}
			} else {
				xId = value;
			...
			//Add the child nodes
				var tagElement = createElement(name, '', relationNode);
				var valueElement = createElement('text', value, tagElement);
		});
		if (xId == "") {
			xId = from + '/' + to;
		...
		//update XML Doc
		for(var i=0;i<extraLinks;i++){
			relations[i].index = edgeIndex+i;
			relations[i].docIndex = total_links;
			forceLinks.push(relations[i]);
		}
		relationNode.id = xId;
		updateXMLDoc("relation", relationNode);
		restart();
...
function updateXMLDoc(type, nodeAppend){
	if(type == "distribution"){		
		var distributions = xmlDoc.getElementsByTagName("distributions")[0];
		distributions.appendChild(nodeAppend);
	}else{
		var relations = xmlDoc.getElementsByTagName("relations")[0];
		relations.appendChild(nodeAppend);
		total_links++;
	}
}
