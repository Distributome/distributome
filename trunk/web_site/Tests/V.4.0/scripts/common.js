...
/******* Create Element with tag name and value and append to parent node *******/
function createElement(element, value, parent){
	if(element == "text"){
		var elementObject = document.createTextNode(value);
	}else{
		var elementObject = eval('document.createElement(\''+element+'\')');
	}
	if(parent){
...
function XMLParser(i, nodeNameIndex, index, reference, XML_Objects, docIndex){
	var html = new Array();
	...
	if (XML_Objects[i].nodeType==1) {
		...
		if(xmlDoc.getElementsByTagName(Level1Prop[nodeNameIndex].nodeName)[index] == undefined){
			var Level2Prop=xmlDoc.getElementsByTagName(Level1Prop[nodeNameIndex].nodeName)[docIndex].childNodes;
			var currLevel2Prop=xmlDoc.getElementsByTagName(Level1Prop[nodeNameIndex].nodeName)[docIndex].firstChild;
		}else{
			var Level2Prop=xmlDoc.getElementsByTagName(Level1Prop[nodeNameIndex].nodeName)[index].childNodes;
			var currLevel2Prop=xmlDoc.getElementsByTagName(Level1Prop[nodeNameIndex].nodeName)[index].firstChild;
		}
...
function traverseXML(searchFlag, text, XML_Objects, nodes, edges, references, nodesArray, referenceArray, connectivity, citeArray, citeLinkArray){
	//alert("traverse");
							...
							try {
								if (currLevel2Prop.nodeType==1) {
									...
									if(searchFlag){
										...
										}else if(citeArray!=undefined && currLevel2Prop.nodeName == "cite" && !citeFlag && trim(value)!=''){
											citeFlag = true;
											citeArray[getDistributionName(value)] = currentNodeIndex;
											citeArray[currentNodeIndex] = trim(value);
										}else if(citeArray!= undefined && currLevel2Prop.nodeName == "cite" && citeFlag && trim(value)!=''){
											citeArray[getDistributionName(value)] = currentNodeIndex;
											citeArray[currentNodeIndex] = citeArray[currentNodeIndex]+ ","+trim(value);
										}
									}
								} else k_corr++;
								currLevel2Prop=currLevel2Prop.nextSibling;
								...
								if (currLevel2Prop.nodeType==1) {
									...
									if(searchFlag){ //Check search
										...
										if (currLevel2Prop.nodeName == "from") {
											...
											edges[currentEdgeIndex].docIndex = currentEdgeIndex;	
											if(totalExtraNodes > 0){
												for(var extraNodes =0; extraNodes<totalExtraNodes; extraNodes++){
													edges[totalEdges+extraNodes].value = type;
													edges[totalEdges+extraNodes].docIndex = currentEdgeIndex;	
													edges[totalEdges+extraNodes].index = totalEdges+extraNodes;
												}
											}
										} else if (currLevel2Prop.nodeName == "cite" && citeLinkArray!=undefined) {
											if(citeLinkFlag){
												citeLinkArray[currentEdgeIndex] = citeLinkArray[currentEdgeIndex] +"," + value;
											}else{
												citeLinkArray[currentEdgeIndex] = value;
												citeLinkFlag = true;
											}
										}
