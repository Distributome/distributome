 ...
 *
 *
 * Written by Greg Ross, modified by SOCR Team
 *
    ...
    hideFlatMinPolygons, tooltipColour, origin, startXAngle, startZAngle, zAxisTextPosition, glOptions, data)
{
   ...
   // var surfaceHandlers = new SurfaceHandlers(mouseDown, mat4, self, rotationMatrix);
    ...
    this.render = function()
    {
        ...
        else // Render not quite so shiny non-GL surface-plot.
        {
                ...
                transformation.init();
                transformation.rotate(20+currentXAngle, 0.0, currentZAngle);
	...
	this.setMouseDown = function(state)
	{
		mouseDown = state;
	}
	
	//Handler for mouseMove (for regular mouse movements)
	    ...
	    this.calculateZScale = function()
    {
    	...
    	var maxTrueAxis = max;
//    	if(max>=.1){
//    		maxTrueAxis = Math.round(100*max)/100;
//    	}
//    	else if(max>=.01){
//    		maxTrueAxis = Math.round(1000*max)/1000;
//    	}
//    	else{
//    		var maxTrueAxis = Math.round(10000*max)/10000;
//    	}
		var labels = [];
		
		var interval =maxTrueAxis/8;
		maxTrueAxis += 2*interval;
		interval =maxTrueAxis/ticks;
		var rounded2dp = 0;
		var prec = 1; 
		while(rounded2dp == 0)
		{
			prec++;
			rounded2dp = (ticks*interval).toFixed(prec-1);
		}
		for (var i = 0; i <= ticks; i++)
		{
			rounded2dp = i * interval;
			labels.push(rounded2dp.toFixed(prec));
		}
