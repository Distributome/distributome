...
function Data(a0, b0, t0){
	...
	this.relFreq = function(x){
		return f[this.index(x)]/n;
	};
	
	this.density = function(x){
		return f[this.index(x)]/(n * t);
	};
...
BernoulliDistribution.prototype = new BernoulliDistribution;
...
function BetaNegativeBinomialDistribution(a0, b0, k0){
	...
	Distribution.call(this, k, Math.round(m + 4 * Math.sqrt(s2)), DISC);
...
//Discrete uniform distribution on the points {a, a + 1, ..., a + n - 1}
function DiscreteUniformDistribution(a0, n0){
	var a, n;
	if (-Infinity < a0 && a0 < Infinity) a = a0; else a = 1;
	if (1 <= n0 && n0 < Infinity) n = Math.round(n0); else n = 1;
	Distribution.call(this, a, a + n - 1, 1, DISC);
	
	this.density = function(x){
		x = Math.round(x);
		if (a <= x && x <= a + n - 1) return 1/n;
		else return 0;
	};
	...
	this.simulate = function(){
		var x = a + Math.floor(n * Math.random());
		this.setValue(x);
		return x;
	};
...
function UQuadraticDistribution(a0, w0){
	var a, w;
	if (-Infinity < a0 && a0 < Infinity) a = a0; else a = 0;
	if (0 < w0 && w0 < Infinity) w = w0; else w = 1;
	Distribution.call(this, a, a + w, w/100, CONT);
	...
	this.density = function(x){
		if (a <= x && x <= a + w) return (12/Math.pow(w, 3)) * Math.pow(x - a - w/2, 2);
		else return 0;
	};
	...
	this.CDF = function(x){
		...
		else return 4 * Math.pow(x - a - w/2, 3)/Math.pow(w, 3) + 1/2;
	};
	
	this.quantile = function(p){
		if (0 <= p && p <= 0.5) return a + (w/2) * (1 - Math.pow(1 - 2*p, 1/3));
		else if (0.5 < p && p <= 1) return a + (w/2) * (1 + Math.pow(2*p - 1, 1/3));
		else return NaN;
	}
...
//General classes of distribuitons
